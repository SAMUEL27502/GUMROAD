...
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement>>(
  ({ className, ...props }, ref) => (
    <div className="border-border/80 relative w-full overflow-auto rounded-xl border">
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        "border-border/60 hover:bg-muted/30 data-[state=selected]:bg-muted/50 border-b transition-colors",
        className
      )}
      {...props}
    />
